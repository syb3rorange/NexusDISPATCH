...
interface Incident {
  ...
  assignedUnits: string; // JSON Stringified array
  logs: string; // JSON Stringified array
  startTime: string;
}
...
const STORAGE_KEY_PROFILE = 'nexus_cad_profile_v6';
const STORAGE_KEY_DISPATCH_AUTH = 'nexus_cad_dispatch_v6';
...
const App: React.FC = () => {
  ...
  // Fix: Explicitly cast values to respective types to resolve "unknown" property errors during iteration
  const units = useMemo(() => Object.values(unitsMap) as Unit[], [unitsMap]);
  const incidents = useMemo(() => (Object.values(incidentsMap) as (Incident | null)[]).filter((i): i is Incident => !!i && i.status === 'ACTIVE'), [incidentsMap]);
  ...
  // REALTIME SYNC (Granular)
  useEffect(() => {
    const root = gun.get('nexus_cad_v7_stable').get(roomId);
  ...
  const handleLoginDispatch = () => {
    if (hasPersistentDispatch || dispatchPass === '10-4') {
      ...
      alert("Unauthorized Access. Passcode 10-4 required.");
    }
  };
  ...
  const performJoin = (data: {roblox: string, callsign: string, type: UnitType}) => {
    ...
    const newUnit: Unit = {
      id: callsign, // Use callsign as durable ID
      name: callsign,
      type: data.type,
    ...
    gun.get('nexus_cad_v7_stable').get(roomId).get('units').get(callsign).put(newUnit);
  };
  ...
  const updateUnitStatus = (unitId: string, status: UnitStatus) => {
    gun.get('nexus_cad_v7_stable').get(roomId).get('units').get(unitId).get('status').put(status);
    gun.get('nexus_cad_v7_stable').get(roomId).get('units').get(unitId).get('lastUpdated').put(new Date().toISOString());
  };
  ...
  const createIncident = async () => {
    ...
    const initialLogs: IncidentLog[] = [{ 
      ...
      sender: 'DISPATCH', 
      message: `Incident Broadcasted: ${newCallType}` 
    }];
    ...
    const newIncident: Incident = {
      ...
      status: 'ACTIVE',
      assignedUnits: JSON.stringify([]), // Array to String for Gun
      logs: JSON.stringify(initialLogs), // Array to String for Gun
      startTime: new Date().toISOString(),
    };
    
    gun.get('nexus_cad_v7_stable').get(roomId).get('incidents').get(id).put(newIncident);
  ...
  const handleAddLog = async () => {
    ...
    if (currentIncident) {
      ...
      gun.get('nexus_cad_v7_stable').get(roomId).get('incidents').get(activeIncidentId).get('logs').put(JSON.stringify([...logs, newLog]));
    }
    setLogInput('');
  };
  ...
  const handleCloseIncident = () => {
    ...
    // Mark as inactive or null to delete
    gun.get('nexus_cad_v7_stable').get(roomId).get('incidents').get(activeIncidentId).put(null);
    ...
    return (
      <div className="h-screen w-screen bg-[#020617] flex flex-col items-center justify-center p-4 text-slate-100 relative overflow-hidden">
        ...
        <div className="z-10 w-full max-w-5xl flex flex-col items-center max-h-full overflow-y-auto py-10 px-4 custom-scrollbar">
          ...
          <div className="flex gap-3 mb-12 text-[10px] font-mono uppercase text-slate-600 shrink-0 tracking-[0.3em]">Operational Frequency: <span className="text-blue-400 font-bold">{roomId}</span></div>
          <div className="grid lg:grid-cols-3 gap-6 w-full max-w-6xl">
            ...
            <div className="bg-slate-900/40 border border-slate-800 p-8 md:p-10 rounded-[2.5rem] backdrop-blur-xl flex flex-col hover:border-blue-500/50 transition-all shadow-xl">
              ...
              {hasPersistentDispatch ? (
                  <div className="mb-6 p-5 bg-blue-500/10 border border-blue-500/30 rounded-2xl flex flex-col items-center justify-center">
                    ...
                    <span className="text-xs text-slate-500 font-mono italic text-center">Identity verified. Tap below to resume command.</span>
                  </div>
              ) : (
              ...
              <button onClick={handleLoginDispatch} className="w-full bg-blue-600 hover:bg-blue-500 p-5 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all shadow-lg mt-auto active:scale-95">Establish Uplink</button>
            </div>
            {/* Field Unit Login */}
            <div className="bg-slate-900/40 border border-slate-800 p-8 md:p-10 rounded-[2.5rem] backdrop-blur-xl flex flex-col hover:border-emerald-500/50 transition-all shadow-xl">
              <h2 className="text-xl font-black mb-6 uppercase flex items-center gap-3"><Icons.Police /> Field Unit</h2>
              <div className="space-y-4 mb-6">
                <input type="text" placeholder="Roblox ID" value={onboardingData.roblox} onChange={(e) => setOnboardingData(p => ({...p, roblox: e.target.value}))} className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 font-bold outline-none focus:ring-2 focus:ring-emerald-500 text-sm shadow-inner" />
                ...
                <div className="grid grid-cols-3 gap-2">
                    {[UnitType.POLICE, UnitType.FIRE, UnitType.EMS].map(t => (
                        <button key={t} onClick={() => setOnboardingData(p => ({...p, type: t}))} className={`py-3 rounded-xl border text-[9px] font-black tracking-tighter transition-all ${onboardingData.type === t ? 'bg-emerald-600 border-emerald-400 text-white' : 'bg-slate-950 border-slate-800 text-slate-600'}`}>{t}</button>
                    ))}
                </div>
              </div>
              <button onClick={handleJoinUnit} className="w-full bg-emerald-600 hover:bg-emerald-500 p-5 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all shadow-lg active:scale-95">Sync Identity</button>
            </div>
            {/* Quick Login */}
            <div className={`bg-slate-900/40 border-2 ${savedProfile ? 'border-blue-500/50 shadow-blue-500/20' : 'border-slate-800/20'} p-8 md:p-10 rounded-[2.5rem] backdrop-blur-xl flex flex-col transition-all relative overflow-hidden shadow-xl min-h-[300px]`}>
               {!savedProfile && <div className="absolute inset-0 bg-slate-950/40 backdrop-grayscale flex items-center justify-center text-[10px] font-black uppercase tracking-widest text-slate-800 italic">No Cached Assets</div>}
               ...
               {savedProfile && (
                 <div className="flex flex-col h-full animate-in zoom-in-95">
                    <div className="bg-slate-950/60 p-6 rounded-2xl border border-slate-800 mb-6 shadow-inner">
                        <div className="text-[10px] font-black text-slate-700 uppercase mb-1 tracking-widest">Active Node</div>
                    ...
                    <button onClick={handleQuickJoin} className="w-full bg-blue-600 hover:bg-blue-500 p-5 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all mt-auto active:scale-95 flex items-center justify-center gap-3">
                        <Icons.Refresh /> Resume Status
                    </button>
                    <button onClick={() => {localStorage.removeItem(STORAGE_KEY_PROFILE); setSavedProfile(null);}} className="text-[9px] text-slate-700 mt-4 font-black uppercase hover:text-red-500 transition-colors text-center">Clear Local ID</button>
                 </div>
               )}
            </div>
          </div>
  ...
  const RenderHeader = () => (
    <header className={`h-16 shrink-0 ${session.role === 'DISPATCH' ? 'bg-slate-900/50 border-blue-500/20' : 'bg-slate-900/50 border-emerald-500/20'} border-b flex items-center justify-between px-4 md:px-8 backdrop-blur-xl z-20`}>
        ...
        <div className="flex items-center gap-2 md:gap-4">
          <button onClick={handleManualRefresh} className={`p-3 rounded-xl border border-slate-800 hover:border-blue-500/50 transition-all ${isSyncing ? 'text-blue-400 bg-blue-500/10' : 'text-slate-500'}`} title="Sync Data">
            <div className={isSyncing ? 'animate-spin' : ''}><Icons.Refresh /></div>
          </button>
          <button onClick={() => setIsMobileMode(!isMobileMode)} className="p-3 rounded-xl border border-slate-800 hover:border-blue-500/50 text-slate-500 transition-all" title="Toggle View">
             {isMobileMode ? <Icons.Monitor /> : <Icons.Smartphone />}
          </button>
          {session.role === 'DISPATCH' && <button onClick={() => setIsCreatingCall(true)} className="bg-blue-600 hover:bg-blue-500 px-4 md:px-6 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all active:scale-95 border border-white/10 shadow-lg">New Broadcast</button>}
          <button onClick={() => { if(session.role==='DISPATCH') localStorage.removeItem(STORAGE_KEY_DISPATCH_AUTH); setSession(null); }} className="text-[10px] font-black uppercase text-slate-600 hover:text-red-500 px-2 transition-colors">Logout</button>
        </div>
      </header>
  );
  ...
  const DesktopUI = () => (
    <div className="flex-1 flex overflow-hidden">
        <aside className="w-80 border-r border-slate-800/60 bg-slate-950/40 flex flex-col shrink-0">
          <div className="p-6 border-b border-slate-800 flex items-center justify-between"><h2 className="text-[10px] font-black uppercase tracking-widest text-slate-500">Personnel Roster</h2></div>
          <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
            {units.map(unit => (
              <div key={unit.id} className={`p-5 rounded-3xl border transition-all ${unit.name === session.callsign ? 'bg-emerald-500/5 border-emerald-500/40 shadow-xl' : 'bg-slate-900/40 border-slate-800/50'}`}>
                <div className="flex justify-between mb-3 items-center">
                  <span className="font-mono font-black text-sm tracking-tight uppercase">{unit.name}</span>
            ...
            {units.length === 0 && <div className="text-center py-20 text-slate-800 font-black text-[10px] uppercase tracking-widest italic opacity-40">Zero Field Assets Online</div>}
          </div>
        </aside>
        ...
        <section className="flex-1 flex flex-col bg-[#020617]">
          <div className="h-44 shrink-0 border-b border-slate-800/60 flex p-6 gap-6 overflow-x-auto items-center custom-scrollbar">
            {incidents.map(incident => (
              <div key={incident.id} onClick={() => setActiveIncidentId(incident.id)} className={`w-80 shrink-0 p-6 rounded-[2.5rem] border cursor-pointer transition-all relative ${activeIncidentId === incident.id ? 'bg-blue-900/5 border-blue-500 shadow-2xl scale-[1.02]' : 'bg-slate-900/30 border-slate-800/50 hover:bg-slate-900/40'}`}>
            ...
            {incidents.length === 0 && <div className="w-full text-center text-slate-800 font-black text-[11px] uppercase tracking-[0.4em] opacity-40 italic">Operational Silence</div>}
          </div>
          
          {activeIncident ? (
            <div className="flex-1 flex flex-col p-8 overflow-hidden animate-in fade-in slide-in-from-bottom-2">
               <div className="flex justify-between items-start mb-10">
                  <div><h2 className="text-5xl font-black text-white uppercase tracking-tighter mb-4 drop-shadow-2xl">{activeIncident.callType}</h2><div className="text-[11px] text-slate-500 uppercase tracking-[0.3em] font-black">Target Location: {activeIncident.location}</div></div>
                  {session.role === 'DISPATCH' && <button onClick={handleCloseIncident} className="bg-red-600/10 hover:bg-red-600 text-red-500 hover:text-white px-10 py-4 rounded-[1.5rem] font-black text-[11px] uppercase tracking-widest transition-all border border-red-500/20 active:scale-95 shadow-xl">Purge Broadcast</button>}
               </div>
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <main className="flex-1 overflow-y-auto bg-[#020617] p-4 custom-scrollbar">
         {mobileTab === 'UNITS' && (
            <div className="space-y-4 animate-in fade-in">
               ...
               {units.map(unit => (
                <div key={unit.id} className={`p-4 rounded-2xl border ${unit.name === session.callsign ? 'bg-emerald-500/5 border-emerald-500/40' : 'bg-slate-900/40 border-slate-800'}`}>
                    <div className="flex justify-between items-center mb-3">
                        <span className="font-mono font-black text-sm uppercase">{unit.name}</span>
                        <div className={`text-[9px] px-2 py-0.5 rounded-lg border font-black ${STATUS_COLORS[unit.status]}`}>{unit.status.replace(/_/g, ' ')}</div>
                    </div>
                    {(session.role === 'DISPATCH' || unit.name === session.callsign) && (
                        <div className="grid grid-cols-5 gap-1">
               ...
               {units.length === 0 && <div className="text-center py-20 text-slate-800 text-[10px] font-black uppercase tracking-widest italic opacity-40">No Units Active</div>}
            </div>
         )}
         {mobileTab === 'INCIDENTS' && (
            <div className="space-y-4 animate-in fade-in">
               <h2 className="text-[10px] font-black uppercase tracking-widest text-slate-500 px-2 mb-4">Frequency Queue</h2>
               {incidents.map(incident => (
                <div key={incident.id} onClick={() => { setActiveIncidentId(incident.id); setMobileTab('ACTIVE'); }} className={`p-5 rounded-2xl border cursor-pointer ${activeIncidentId === incident.id ? 'bg-blue-900/5 border-blue-500 shadow-xl' : 'bg-slate-900/40 border-slate-800'}`}>
                    ...
                    <div className="text-xs text-slate-500 mt-1 italic">Loc: {incident.location}</div>
                </div>
               ))}
               {incidents.length === 0 && <div className="text-center py-20 text-slate-700 font-black text-[10px] uppercase tracking-widest opacity-30 italic">Operational Silence</div>}
            </div>
         )}
         {mobileTab === 'ACTIVE' && (
                ...
                {activeIncident ? (
                    <div className="flex flex-col h-full space-y-4 overflow-hidden">
                        <div className="p-4 bg-slate-900/40 border border-slate-800 rounded-2xl shrink-0">
                            ...
                            <div className="text-[10px] text-slate-500 uppercase tracking-widest italic">Target: {activeIncident.location}</div>
                            {session.role === 'DISPATCH' && <button onClick={handleCloseIncident} className="w-full mt-4 bg-red-600/10 hover:bg-red-600 text-red-500 hover:text-white py-3 rounded-xl font-black text-[10px] uppercase tracking-widest border border-red-500/20 shadow-lg active:scale-95 transition-all">Abort Incident</button>}
                        </div>
                        <div className="flex-1 bg-slate-950/40 rounded-2xl border border-slate-800 overflow-hidden flex flex-col shadow-2xl min-h-0">
                            <div className="flex-1 overflow-y-auto p-4 space-y-4 font-mono text-xs custom-scrollbar">
                                {(() => {
                                  ...
                                  try { logs = JSON.parse(activeIncident.logs); } catch(e) {}
                                  return logs.map((log, idx) => (
                                    <div key={idx} className="flex gap-3"><span className="text-slate-800 font-black text-[9px] shrink-0 mt-0.5">[{log.timestamp.split(' ')[0]}]</span><div><span className={`font-black uppercase text-[10px] mr-2 ${log.sender === 'DISPATCH' ? 'text-blue-500' : 'text-emerald-500'}`}>{log.sender}:</span><span className="text-slate-400">{log.message}</span></div></div>
                                  ));
                                })()}
                            </div>
                            <div className="p-4 bg-slate-950/60 border-t border-slate-800 flex gap-2 shrink-0">
                                <input type="text" value={logInput} onChange={(e) => setLogInput(e.target.value)} placeholder="Situational report..." className="flex-1 bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-xs font-bold outline-none text-white focus:ring-1 focus:ring-blue-500 transition-all shadow-inner" />
      ...
      <nav className="h-20 bg-slate-950 border-t border-slate-900 grid grid-cols-3 shrink-0 backdrop-blur-xl">
         ...
         <button onClick={() => setMobileTab('ACTIVE')} className={`flex flex-col items-center justify-center gap-1 transition-all ${mobileTab === 'ACTIVE' ? 'text-blue-500' : 'text-slate-700'}`}>
            <Icons.Plus /><span className="text-[9px] font-black uppercase tracking-widest">Mission</span>
         </button>
      </nav>
    </div>
  );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#020617]/95 backdrop-blur-xl p-4 md:p-8">
          <div className="bg-slate-900 border border-slate-800 rounded-[3rem] p-8 md:p-12 w-full max-w-2xl space-y-8 animate-in zoom-in-95 shadow-3xl max-h-[90vh] overflow-y-auto custom-scrollbar">
             <div className="grid md:grid-cols-2 gap-6 md:gap-8">
                <div className="space-y-4"><label className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Incident Category</label><select value={newCallType} onChange={(e) => setNewCallType(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 md:p-5 font-black text-white outline-none appearance-none cursor-pointer transition-all hover:border-slate-700 shadow-inner">{CALL_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
             ...
             <div className="space-y-4"><label className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Deployment Location</label><input type="text" placeholder="STREET / ZONE / GRID / COORDINATES" value={newLocation} onChange={(e) => setNewLocation(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 md:p-6 font-black outline-none focus:ring-2 focus:ring-blue-500 text-white shadow-inner transition-all placeholder:text-slate-800" /></div>
             <div className="flex gap-4 md:gap-6 pt-4">
                <button onClick={() => setIsCreatingCall(false)} className="flex-1 font-black text-[11px] text-slate-500 uppercase tracking-widest hover:text-white transition-colors">Abort</button>
      ...
      <footer className="h-10 md:h-12 bg-slate-950 border-t border-slate-900 flex items-center px-4 md:px-8 justify-between shrink-0 text-[10px] font-mono tracking-widest text-slate-700 uppercase font-black z-20">
        <div className="flex gap-4 md:gap-10 items-center">
          <div className="flex items-center gap-2 md:gap-3">
             ...
             SYNC_STATUS: {isSyncing ? 'UPDATING' : 'ONLINE'}
          </div>
          <div className="hidden sm:flex items-center gap-3 text-slate-800 italic">FREQ_ID: {roomId.toUpperCase()}</div>
        </div>
        <div className="flex items-center gap-4">
          <div className="text-slate-800 font-black hidden xs:block">NEXUS_v5.9.0 // PRODUCTION_STABLE</div>
        </div>
      </footer>
    </div>
  );
